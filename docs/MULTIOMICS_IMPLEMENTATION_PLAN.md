...
Ultimate 的目标是构建一个给 Codex 使用的生产级个人多组学生信分析 workbench，而不是单一 scRNA pipeline，也不是高度自动化的 agent 决策系统。

实际使用方式是：用户提供原始数据路径、已有样本信息和自然语言需求，Codex 使用 Ultimate 的模块库、工具注册表、配置模板、Slurm wrapper、报告模板和复现包来判断该走哪些模块和工具路线。用户不需要预先指定所有数据类型、模块或工具；Ultimate 负责把 Codex 的判断变成可复查、可执行、可复现的工作流。
...
所有实现和远程操作必须遵守本地控制仓库根目录 `AGENTS.md`。远端正式 Ultimate 项目根目录固定为 `/shared/shen/2026/ultimate`。
