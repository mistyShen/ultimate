...
def _sanitize_text(text: str) -> str:
    ...
    sanitized = re.sub(r"\bSLURM_JOB_ID\b\s*[:=]?\s*\S*", "slurm_job_id: [redacted]", sanitized, flags=re.IGNORECASE)
    sanitized = re.sub(r"slurm_job_id\s*[:=]\s*[A-Za-z0-9_.-]+", "slurm_job_id: [redacted]", sanitized, flags=re.IGNORECASE)
