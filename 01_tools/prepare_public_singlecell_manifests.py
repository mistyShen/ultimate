...
PUBLIC_DATASETS = {
    "scrna_pbmc3k": {
        "title": "10x PBMC3k public scRNA matrix",
        "status": "manifest_only",
        "required_inputs": ["filtered_gene_bc_matrices/hg19/matrix.mtx", "filtered_gene_bc_matrices/hg19/barcodes.tsv", "filtered_gene_bc_matrices/hg19/genes.tsv"],
        "optional_inputs": ["processed h5ad generated by validate-scrna"],
        "source_note": "用于 scRNA MVP validated_backend；下载失败时只记录 partial:data_required，不生成假数据。",
    },
    "scatac": {
...
def main() -> None:
    ...
    for key, payload in PUBLIC_DATASETS.items():
        ...
        if args.mode == "download" and key == "scrna_pbmc3k":
            download_result = _download_pbmc3k(out_dir)
        ...
        if download_result and download_result["status"] == "ready":
            ...
            validation_status = "partial:data_required"
        else:
...
def _download_pbmc3k(out_dir: Path) -> dict:
    result = _download_files(out_dir, SCRNA_PBMC3K_DOWNLOADS, "10x PBMC3k filtered gene-barcode matrix")
    if result["status"] != "ready":
        return result
    archive = out_dir / "pbmc3k_filtered_gene_bc_matrices.tar.gz"
    matrix_dir = out_dir / "filtered_gene_bc_matrices" / "hg19"
    expected = (matrix_dir / "matrix.mtx", matrix_dir / "barcodes.tsv", matrix_dir / "genes.tsv")
    try:
        ...
        if not all(path.exists() and path.stat().st_size > 0 for path in expected):
            return {
                ...
                "description": "10x PBMC3k filtered gene-barcode matrix",
                "downloaded": result.get("downloaded", {}),
                "errors": {"extract": "expected matrix.mtx/barcodes.tsv/genes.tsv not found after extraction"},
                "reason": "公开 PBMC3k 数据已下载但未能解压出 10x matrix 目录。",
            }
        ...
        return {
            ...
            "description": "10x PBMC3k filtered gene-barcode matrix",
            "downloaded": result.get("downloaded", {}),
            "errors": {"extract": f"{type(exc).__name__}: {exc}"},
            "reason": "公开 PBMC3k 数据下载后解压失败。",
        }
    result["matrix_dir"] = str(matrix_dir)
    result["required_inputs"] = [str(path) for path in expected]
    return result
