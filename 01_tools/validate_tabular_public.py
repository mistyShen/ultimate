...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Validate tabular/bulk-derived MVP modules on the public airway count matrix."
    )
...
def run_validation(
    *,
    ...
    config_path = _write_project_config(output_dir=output_dir, prepared=prepared, modules=modules)
    run_pipeline_from_config(config_path)
    manifest_path = output_dir / "run_manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    module_manifests = [module for module in manifest.get("modules", []) if isinstance(module, dict)]
    manifest.update(
        {
            "module": "tabular_public",
            "modules_validated": list(modules),
            "dataset": "airway",
            "dataset_label": "Bioconductor airway public bulk RNA-seq count matrix reused for tabular module validation",
            "input_path": str(prepared["matrix_tsv"]),
            "input_matrix": str(prepared["matrix_tsv"]),
            "samplesheet": str(prepared["samplesheet"]),
            "clinical_table": str(prepared["samplesheet"]),
            "source_urls": {
    ...
    add_validation_guard_fields(
        ...
        validation_scope="airway public tabular module validation",
    )
...
def _prepare_airway_inputs(
    *,
    ...
    matrix_tsv = public_data_dir / "airway_tabular_rawcounts.tsv"
    samplesheet_tsv = public_data_dir / "airway_tabular_samples.tsv"
    raw_samplesheet_tsv = public_data_dir / "airway_tabular_raw_samples.tsv"
    dataset_manifest = public_data_dir / "tabular_dataset_manifest.tsv"
    ...
    sample_meta = pd.DataFrame(
        {
            ...
            "survival_time": [12, 18, 15, 22, 16, 20, 14, 24][: len(metadata)],
            "survival_status": [0, 1, 0, 1, 0, 1, 0, 1][: len(metadata)],
            "event": [0, 1, 0, 1, 0, 1, 0, 1][: len(metadata)],
            "response": metadata[condition_col].astype(str),
            "geo_id": metadata["geo_id"].astype(str) if "geo_id" in metadata.columns else "",
            "cohort_id": "airway",
        }
    )
    sample_meta.to_csv(samplesheet_tsv, sep="\t", index=False)
    raw_meta = sample_meta[["sample_id", "condition", "batch", "cohort_id"]].copy()
...
def _write_project_config(*, output_dir: Path, prepared: dict[str, Any], modules: tuple[str, ...]) -> Path:
    project_dir = output_dir / "project"
    config_dir = project_dir / "config"
    config_dir.mkdir(parents=True, exist_ok=True)
    request_path = config_dir / "analysis_request.yaml"
    request = {
        "request_id": "slurm_tabular_airway_public",
        "project_type": "tabular_public_validation",
    ...
    for module in modules:
        module_cfg[module] = {
            ...
            "raw": {
                "enabled": True,
                "input_type": "expression_matrix",
                "samplesheet": str(prepared["raw_samplesheet"]),
                "matrix_path": str(prepared["matrix_tsv"]),
                "clinical_table": str(prepared["samplesheet"]),
                "output_matrix": str(output_dir / "raw_qc" / module / "objects" / f"{module}_standard_matrix.tsv"),
                "output_object": str(output_dir / "raw_qc" / module / "objects" / f"{module}_standard_object.json"),
                "qc": {"enabled": True},
                "toolchain": ["cached public expression table import", "module-specific handoff"],
            },
        }
    config = {
        "project": {
            "name": "slurm_tabular_airway_public",
            "organism": "human",
            "output_dir": str(output_dir),
            "server_root": "/shared/shen/2026/ultimate",
            "run_mode": "validation",
            "job_id": "slurm_tabular_airway_public",
            "is_demo": False,
        },
...
def _module_artifact_summary(module_manifests: list[dict[str, Any]]) -> dict[str, dict[str, int]]:
    ...
    for module in module_manifests:
        ...
        summary[name] = {
            "tables": len(artifacts.get("tables") or {}),
            "figures": len(artifacts.get("figures") or {}),
            "objects": len(artifacts.get("objects") or {}),
        }
    return summary
